/**
 * Counterexamples for the pieces that turn a red replay into a focused repair.
 *
 ...
 * the list a repair run must touch. Both are deterministic code; if either
 * breaks, a repair silently degrades back to re-running every green spec to find
 * the red one, which is exactly the two-hour waste this exists to avoid.
 */
...
test('reporter: records each spec file with its verdict alongside the steps', () => {
  ...
  try {
    ...
    r.onTestEnd({ location: { file: 'run/people/search.spec.ts' } }, { status: 'failed' });
    r.onStepEnd({ title: 'S' }, {}, {
      category: 'test.step', title: 'Given a page',
      location: { file: 'run/people/search.spec.ts', line: 5 }, error: null, steps: [],
    });
    r.onStepEnd({ title: 'S' }, {}, { category: 'pw:api', title: 'click' });
    r.onEnd();

    const parsed = JSON.parse(readFileSync(out, 'utf8'));
    assert.deepEqual(parsed.tests, [{ file: 'run/people/search.spec.ts', status: 'failed', error: null }]);
...
test('red specs: an empty run leaves nothing to repair', () => {
